...
var prefixEffect = " > ";
var textColor = {
	PositivityText:"green;", // Позитивные Эффекты		Только к тексту описания эффекта,оглавнеие не идёт в данный список
	NegativityText:"red;", // Негативные Эффекты 
	...
	var effect_list = CustomNetTables.GetTableValue(KeyTable,KeyTableEffects);
		...
		if (effect_list[name]){
			...
			for (i in negativity){
				...
				NegativeEffectLabel.style.color = "red;";
				l++;
			}
		}
...
function PickedHero( data,skip ){
	return function(){
		...
		if (GetPickTime() <= 0){
			...
			GameEvents.SendCustomGameEventToServer( "SetClassHero", data );
			FindDotaHudElement('ButtonPickHero').enabled = true;
			FindDotaHudElement('ButtonRepickHero').enabled = true;
			UnHiddenOther();
			HidePanel(Main.GetParent()); 
		}else{
			$.Schedule(1,PickedHero(data,true));	
			 FindDotaHudElement('ButtonPickHero').enabled = false;
			 FindDotaHudElement('ButtonRepickHero').enabled = false;
		}
	};
}
...
function LockedPanels(data){
	var table = data.table;
	var PanelLocked;
	for (i in table){
		PanelLocked = FindDotaHudElement("MainPickHero").FindChildTraverse(table[i]);
		if (PanelLocked && !PanelLocked.customLocked){
			PanelLocked.enabled = false;
			PanelLocked.hittest = false;
			PanelLocked.customLocked = true;
		}
	}
}
(function(){
	...
	GameEvents.Subscribe('PickingHeroes', LockedPanels);
})(); 